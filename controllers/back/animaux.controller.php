...
class AnimauxController {
    ...
    public function creationTemplate(){
        if(Securite::verifAccessSession()){
            ...
            $continents = $continentManager->getContinents();
    ...
    public function creationValidation(){
        if(Securite::verifAccessSession()){
            ...
            $_SESSION['alert'] = [
                "message" => "L'animal a bien été créé avec l'identifiant : ".$idAnimal,
                "type" => "alert-success"
            ];
    ...
    public function modification($idAnimal){
        if(Securite::verifAccessSession()){
            ...
            $tabContinents = [];
            foreach($lignesAnimal as $contient){
                $tabContinents[] = $contient['continent_id'];
            }
    ...
    public function modificationValidation(){
        if(Securite::verifAccessSession()){
            ...
            foreach ($contients as $key => $contient){
                ...
                if($contients[$key] && !$continentManager->verificationExisteAnimalContinent($idAnimal,$key)){
            ...
            $_SESSION['alert'] = [
                "message" => "L'animal : \"".$nom."\" a bien été modifié ! ",
                "type" => "alert-success"
            ];
